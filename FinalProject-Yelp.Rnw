...
<<ModelDef, echo=FALSE, cache=TRUE>>=
    ...
    reviews.rf <- randomForest(factor(useful_bin) ~ stars+numChar+numCap+numPunc+numPar+hasDont+hasTime+useful.per+average_stars, data = reviews.train, importance = TRUE, ntree = 1000)
