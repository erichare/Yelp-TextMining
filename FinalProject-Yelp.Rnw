...
<<SummaryTable, echo=FALSE, results='asis', cache=TRUE>>=
    ...
    summary.sub <- ddply(reviews.sub, .(useful_bin), summarise, numChar = mean(numChar), numCap = mean(numCap), numPunc = mean(numPunc), numPar = mean(numPar), hasDont = mean(hasDont), hasTime = mean(hasTime))
